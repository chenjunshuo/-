...
static void do_cwd (session_t *sess)//更改公共路径
{
	if(chdir(sess->arg) < 0)//int chdir(const char *path);函数作用：更改路径
	{
...
static void do_mkd (session_t *sess)//建立目录
{
...
static void do_rnfr (session_t *sess)//文件重命名——获取原文件
{
...
static void do_rnto (session_t *sess)//文件重命名——更改文件名
{
...
static void do_size(session_t *sess)//文件大小
{
	struct stat sbuf;
	if(stat(sess->arg, &sbuf) < 0)//stat获取文件信息
	{
	...
	if(!S_ISREG(sbuf.st_mode))//判断sbuf是否是普通文件
	{
		ftp_reply(sess, FTP_FILEFAIL, "Could not get file size.");
		return;
	}
...
static void do_stor(session_t *sess)
{
	...
	//断点续传
	long long offset = sess->restart_pos;//记录偏移量
	sess->restart_pos = 0;
	if(lseek(fd, offset, SEEK_SET) < 0)//lseek(fd 文件指针, offset 偏移量, SEEK_SET 起始位置)
	{
...
static void do_retr(session_t *sess)
{
	...
	int fd = open(sess->arg, O_RDONLY);//打开文件
	if(fd == -1)
	{
		ftp_reply(sess, FTP_FILEFAIL, "Failed to open file.");
		return;
	...
	else
	{
		...
		while(1)
		{
			...
			//文件大小不足MAX_BUFFER_SIZE按文件大小读，超过则按MAX_BUFFER_SIZE读
			ret = read(fd, buf, read_count);

			if(ret==-1 || ret!=read_count)
			{
				ftp_reply(sess, FTP_BADSENDFILE, "Failure reading from local file.");
				break;//break而不能return，因为连接没断开
			}
...
static void do_rest(session_t *sess)//定位续传，续载断点
{
