...
static void check_limit(session_t *sess);//检查限制数
...
static void check_limit(session_t *sess)//检查限制数
{
	...
	//  有每ip最大连接数限制       目前ip连接数 > 每ip最大连接数限制
...
//当有客户端退出，减少连接数
void handle_sigchld(int sig)//信号处理
{
	pid_t pid;
	while((pid = waitpid(-1, NULL, WNOHANG)) > 0)//持续等待客户退出
	{
		--s_children;//减少最大连接数
...
unsigned int hash_func(unsigned int buckets, void *key)//哈希表
{
	return (*(unsigned int*)key % buckets);
}

unsigned int handle_ip_count(void *ip)//每ip连接数处理
{
	int count = 0;
